...
使用相同的模型、数据集、batch size 和 5 个 epoch 进行对比：

| 设备 | 训练时间 | 最终测试准确率 |
|---|---:|---:|
| CPU | 25.68 秒 | 90.73% |
| NVIDIA RTX 4090 | 8.22 秒 | 91.43% |
...
因此在本次实验中，RTX 4090 将训练时间从 25.68 秒降低到了 8.22 秒。两次训练的准确率略有差异，主要来自数据打乱和参数初始化的随机性；速度对比应关注训练时间，而不是把两次不同随机训练的准确率差异归因于硬件。
